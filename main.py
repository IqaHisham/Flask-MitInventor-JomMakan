...
@app.route("/")
def mainpage():
  return render_template("dashboard.html", table=data)
...
@app.route("/success/table/<table_id>")
def get_table_success(table_id):
    resp = jsonify(data[table_id])
    resp.status_code = 200
    return render_template("success.html", table=data[table_id])
...
def order(table_id):
    if request.method == "POST":
        ...
        return get_table_success(table_id)

    if data[table_id]["status"] == 'Available':
        return render_template("menu2.html", table_id=table_id,foodList=food, baverageList=beverage )
    if data[table_id]["status"] == 'Ordered':
        return get_table_success(table_id)
